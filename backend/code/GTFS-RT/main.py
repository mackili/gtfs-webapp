...
if opts.batchSize is None:
    logging.warning(f"No batch size specified. Proceeding with default {BATCH_SIZE}")
else:
...
def send_requests(feed_name) -> None:
    match feed_name:
        case "trip_updates":
            insert(feed_name, trip_updates_dict)
        case "stop_time_updates":
            insert(feed_name, stop_time_updates_dict)
        case "vehicle_positions":
            insert(feed_name, vehicle_positions_dict)
        case "alerts":
            insert(feed_name, alerts_dict)
        case "entity_selectors":
            insert(feed_name, entity_selectors_dict)
